...
const ConnectBtn: React.FC = () => {
  ...
  const handleConnect = useCallback(async () => {
    ...
    setIsModalOpen(true);
    try {
      toast.success({message:"pop up",options:{position:"top-center",autoClose:400}});

      await connectAsync({ connector: metaMaskConnector });
      setUserRejected(false);
      toast.success({message:"connect wallet",options:{position:"top-center"}});
    } catch (error: any) {
      ...
      if (error?.code === 4001 || error?.message?.includes('User rejected')) {
        ...
        setUserRejected(false);
        setIsModalOpen(false);
      toast.error({message:"Wallet failed",options:{position:"top-right"}});
