...
const AuctionCollection = ({ nft }: { nft: NFT }) => {
  ...
  const { bidHistory, loading } = useSelector((s: RootState) => s.buyOrBid);
  ...
  const { tokenId, seller, highestBid, claimed } = nft;

  const lowerSeller = useMemo(() => seller.toLowerCase(), [seller]);
  ...
  const isSeller = useMemo(() => nft.seller.toLowerCase() === address?.toLowerCase(), [nft.seller, address]);
  const canClaim = useMemo(() => (myBid && !myBid.claim) || (isSeller && !claimed), [myBid, isSeller, claimed]);
  ...
  const onPlaceBid = useCallback(async () => {
    if (!bidAmount || Number(bidAmount) <= 0) return toast.info({message:'Enter valid bid',options:{position:"top-center"}});
    if (totalWei <= BigInt(highestBid)) return toast.info({message:'Increase your bid',options:{position:"top-center"}});
    ...
    toast.success({message:`Your Bid ${bidAmount} ETH`});
    setBidAmount('');
  }, [dispatch, tokenId, seller, address, bidAmount, totalWei, highestBid, toast]);

  const onClaim = useCallback(async () => {
    await dispatch(claimAuction({ tokenId: Number(tokenId), seller }));
    toast.success({message:'NFT Claimed!'});
  ...
  const { title, action, disable } = useMemo(() => {
    if (isSeller) {
      if (isAuctionEnded) {
        if (claimed) return { title: 'Auction Ended', disable: true };
        return { title: 'Claim NFT', disable: false, action: onClaim };
      }
      return { title: 'Waiting for Bids', disable: true };
    }

    if (isAuctionEnded) {
      if (canClaim) return { title: 'Claim NFT', disable: false, action: onClaim };
      return { title: 'Auction Ended', disable: true };
    }

    return { title: 'Place Bid', disable: false, action: onPlaceBid };
  }, [isSeller, isAuctionEnded, claimed, canClaim, onClaim, onPlaceBid]);
      ...
      <div className="relative bg-[#151c36]/80 backdrop-blur-xl border border-[#6934d3]/40 rounded-2xl shadow-[0_0_25px_rgba(105,52,211,0.3)] p-6 md:p-8 mb-8 transition-all duration-300 hover:shadow-[0_0_35px_rgba(105,52,211,0.5)]">
        ...
        <div className="bg-[#1f2847]/60 border border-[#6934d3]/30 rounded-xl p-4 mb-6 space-y-1">
          ...
          <p className="text-gray-400 text-xs break-all">
            Highest Bidder: <span className="text-purple-400">{nft.highestBidder}</span>
          </p>
        </div>
